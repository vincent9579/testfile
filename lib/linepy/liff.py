...
class Liff:
    ...
    @loggedIn
    def sendLiffMessage(self,message=None, data=None, liffToken=None, revokeToken=False):
        if liffToken:
        ...
        if 'message' in resp and not self.resend and self.to:
            ...
            return self.sendLiffMessage(message=None, data=data, revokeToken=revokeToken)
        if revokeToken:
            try:
                self.revokeToken(self.liffToken)
                self.liffToken = None
    ...
    @loggedIn
    def sendFlexMessage(self,to,flexContent, altText='Hello World', liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + self.liffToken)
        ...
        return self.sendLiffMessage(data=data,liffToken=liffToken,revokeToken=revokeToken)
    
    @loggedIn
    def sendStickerTemplate(self,to,stickerID,packageID,liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
    ...
    @loggedIn
    def sendMessageWithFooterTemplate(self,to,text,title=None,link=None,iconlink=None,isHelper=False,liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        footer = self.generateFooterTemplate(title,link,iconlink,isHelper)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + self.liffToken)
    ...
    @loggedIn
    def sendImageWithFooterTemplate(self,to,ocu,piu=None,title=None,link=None,iconlink=None,liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
            ...
            raise Exception('sendLiffMessage Failed (you must issueLiffView before send)')
        piu = piu if piu else ocu
        footer = self.generateFooterTemplate(title,link,iconlink)
        data = {
            "messages": [{
                "type": "image",
    ...
    @loggedIn
    def sendImageWithTemplate(self,to,ocu,piu=None,liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
        ...
        return self.sendLiffMessage(data=data,liffToken=None, revokeToken=False)

    @loggedIn
    def sendVideoWithTemplate(self,to,ocu,piu,liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
    ...
    @loggedIn
    def sendAudioWithTemplate(self,to,ocu,liffToken=None, revokeToken=False):
        self.issueLiffView(to)
        if liffToken:
            self.server.setLiffHeaders('Authorization', 'Bearer ' + liffToken)
        elif self.liffToken:
