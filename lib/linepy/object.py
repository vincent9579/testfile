...
class Object(object):
    ...
    def __init__(self):
        if self.isLogin == True:
            self.log("[Display Name] : %s" % self.profile.displayName)
            self.log("[Mid] : %s" % self.profile.mid)
            self.log("[Auth Token] : %s" % self.authToken)
    ...
    @loggedIn
    def testUpdateCover(self,mid,path):
        objId = self.forceUploadObjHome(mid,path, type='image', returnAs='objId')
        print(objId)
            
    @loggedIn
    def forceUpdateCover(self, mid, path, returnAs='bool'):
        if returnAs not in ['objId','bool']:
            raise Exception('Invalid returnAs value')
        objId = self.forceUploadObjHome(mid,path, type='image', returnAs='objId')
        home = self.updateProfileCoverById(mid,objId)
        print(home)
        if returnAs == 'objId':
            return objId
        elif returnAs == 'bool':
    ...
    @loggedIn
    def forceUploadObjHome(self, mid, path, type='image', returnAs='bool', objId=None):
        if returnAs not in ['objId','bool']:
            raise Exception('Invalid returnAs value')
        if type not in ['image','video','audio']:
            raise Exception('Invalid type value')
        ...
        if r.status_code != 201:
            raise Exception('Upload object home failure.')
        if returnAs == 'objId':
            return {"objid":objId,"params":params}
        elif returnAs == 'bool':
            return True
