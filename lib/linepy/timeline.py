...
class Timeline(Channel):
    ...
    def getPost(self, postId, mid=None, commentLimit=1, likeLimit=1):
        ...
        url = self.server.urlEncode(self.server.LINE_TIMELINE_API, '/v44/post/get.json', params)
        r = self.server.getContent(url, headers=self.server.timelineHeaders)
        return r.json()
    ...
    @loggedIn
    def createComment(self, mid, postId, text, metadata=[],contentsList=[]):
        if mid is None:
            mid = self.profile.mid
        ...
        data = {'commentText': text, 'recallInfos': metadata, 'activityExternalId': postId, 'actorId': mid,'contentsList':contentsList}
    ...
    @loggedIn
    def reportPost(self, homeid, postid, typs="R0003"):
        """
        ADVERTISING("ROO14"),
        ...
        OTHER("R0019");
        """
        try:params={"homeId": homeid, "sourceType": "TIMELINE"}
        except:params={"homeId": id, "sourceType": "TALKROOM"}
        url = self.server.urlEncode(self.server.LINE_TIMELINE_API,'/v45/post/report.json',params)
        data = {"postId": postid, "reason": typs}
    ...
    def createGroupPost(self, mid, text):
        ...
        payload = {'postInfo': {'readPermission': {'type':'ALL'}}, 'sourceType': 'TIMELINE', 'contents': {'text': text}}
        data = json.dumps(payload)
        url = self.server.LINE_TIMELINE_API + '/v45/post/create.json?%s' % urllib.parse.urlencode(params)
        r = self.server.postContent(url,data=data, headers=self.server.timelineHeaders)
        return r.json()
    ...
    @loggedIn
    def getGroupPost(self, mid, postLimit=30, commentLimit=1, likeLimit=1):
